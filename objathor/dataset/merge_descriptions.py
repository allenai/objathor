...
def extract_json(ans):
    ...
    try:
        if "```json" in ans:
            source = ans.split("```json")[1].split("```")[0].strip()
        elif "```" in ans:
            source = ans.split("```")[1].strip()

        # C-style single-line comments in a few json dicts
        source = "\n".join([line.split("//")[0] for line in source.split("\n")])

        return json.loads(source)

    except json.JSONDecodeError:
        print(f"Failed to decode with {source=}")
        return None
...
Using up to five words, describe the object in simple words keeping the essentials of the visual appearance (e.g. the dominant color) in the context of some indoor space. Use at least one noun. Prefer about three words if possible and do not add any additional comments.
    """.strip()
